...
export class DetailPtrojetComponent implements OnInit {
  ...
  getData1() {
    ...
    this.uploadService.geLivrableByIdPr(this.id).subscribe(data => {
    this.lienLivrable = data
  ...
  sendClientAdmin() {
    this.dialgo.open(WindowComponent, {
      context: {
        ...
        testid: this.detpr['reference'],
        etat: this.detpr['etat'],
        idSend: this.detpr['userId'], 
        idRec: localStorage.getItem("idUser"),
        aff: '2'
      },
    });
  }
